...
try:
    ...
    cur = conn.cursor()
    cur.execute('SELECT id, user_id FROM officers WHERE user_id IS NOT NULL LIMIT 1')
    row = cur.fetchone()
    if not row:
        print('No officers with user_id found in DB')
    else:
        officer_id_db, user_id_db = row
        print(f'Using officer -> officer_id: {officer_id_db}, user_id: {user_id_db}')
        try:
